...
namespace Saklient.Cloud.Errors
{
	...
	public class BootFailureByLockException : HttpServiceUnavailableException
	{
		...
		/// <param name="message" />
		/// </summary>
		public BootFailureByLockException(long status, string code=null, string message="") : base(status, code, string.IsNullOrEmpty(message) ? "サービスが利用できません。サーバが予期せず終了したため、ロックされています。しばらく時間をおいてから再度お試しください。" : message)
		{
			/*!base!*/;
		}
