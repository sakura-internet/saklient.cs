...
namespace Saklient.Cloud.Errors
{
	...
	public class OperationTimeoutException : HttpServiceUnavailableException
	{
		...
		/// <param name="message" />
		/// </summary>
		public OperationTimeoutException(long status, string code=null, string message="") : base(status, code, string.IsNullOrEmpty(message) ? "サービスが利用できません。操作がタイムアウトしました。サーバが混雑している可能性があります。" : message)
		{
			/*!base!*/;
		}
